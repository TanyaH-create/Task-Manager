...
// GET /api/auth/dash - Fetch user tasks and progress
 router.get("/dash", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    //Extract user ID from authenticated requst
    const userId = req.user?.id;

    if (!userId) {
      return res.status(400).json({ message: "User not found" }); // Check if user is authenticated
    }
    ...
    //Send tasks and progress data back to client
    return res.json({ tasks, progress });
  } catch (error) {
    console.error("Error fetchig Tasks", error)
